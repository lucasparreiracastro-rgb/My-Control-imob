...
const Properties: React.FC<PropertiesProps> = ({ properties, onAddProperty, onDeleteProperty, onUpdateProperty }) => {
  ...
  const handleEditProperty = (e: React.MouseEvent, property: Property) => {
    ...
    setImageSearchQuery(property.title);
  ...
  const handleSaveDescription = () => {
    ...
    const updatedProperty = { ...viewingProperty, description: tempDescription };
    onUpdateProperty(updatedProperty);
  ...
  const handleSaveDate = (index: number) => {
    ...
    if (record.checkIn) record.checkIn = newDateStr;
    const updatedProperty = { ...viewingProperty, rentalHistory: updatedHistory };
    onUpdateProperty(updatedProperty);
    setViewingProperty(updatedProperty);
    setEditingDateIndex(null);
  };
  ...
  const handleAddManualRecord = (e: React.FormEvent) => {
    ...
    if (manualRecord.checkOut) formattedCheckOut = formatDate(manualRecord.checkOut);
          ...
          >
            ...
            <div className="p-5">
              ...
              <div className="flex items-center justify-between border-t pt-4 text-gray-600 text-sm">
                <div className="flex items-center gap-1"><Bed size={16} /><span>{property.bedrooms}</span></div>
                <div className="flex items-center gap-1"><Bath size={16} /><span>{property.bathrooms}</span></div>
                <div className="flex items-center gap-1"><Square size={16} /><span>{property.area} m²</span></div>
              </div>
            </div>
          </div>
        ))}
               ...
               <div>
                ...
                <div className="flex items-center gap-3 text-sm text-gray-500 mt-1">
                    ...
                    {viewingProperty.consumerUnit && (
                        <span className="flex items-center gap-1 bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full text-xs font-medium border border-yellow-200">
